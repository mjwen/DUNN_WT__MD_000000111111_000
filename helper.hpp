...
// deallocate memory
template <class T> void Deallocate1DArray(T *&arrayPtr) {
  delete[] arrayPtr;
  // nullify pointer
  arrayPtr = nullptr;
}
...
// deallocate memory
template <class T> void Deallocate2DArray(T **&arrayPtr) {
  if (arrayPtr != nullptr) {
    delete[] arrayPtr[0];
  }
  delete[] arrayPtr;
...
// deallocate memory
template <class T> void Deallocate3DArray(T ***&arrayPtr) {
  if (arrayPtr != nullptr) {
    if (arrayPtr[0] != nullptr) {
      delete[] arrayPtr[0][0];
    }
    delete[] arrayPtr[0];
  }
