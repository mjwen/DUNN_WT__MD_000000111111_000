...
Descriptor::Descriptor()
    : rcut_2D_(nullptr), has_three_body_(false), normalize_(true) {
  return;
}
...
void Descriptor::add_descriptor(char const *name, double const *values,
...
void Descriptor::set_feature_mean_and_std(bool normalize, int const size,
                                          double const *means,
                                          double const *stds) {
...
void Descriptor::generate_one_atom(const int i,
                                   ...
                                   const int *neighlist, const int numnei,
                                   double *const desc, double *const grad_desc,
                                   bool grad) {
...
void Descriptor::sym_g2(double eta, double Rs, double r, double rcut,
                        double &phi) {
...
void Descriptor::sym_g4(double zeta, double lambda, double eta, const double *r,
                        const double *rcut, double &phi) {
  double rij = r[0];
  double rik = r[1];
  double rjk = r[2];
...
void Descriptor::sym_d_g4(double zeta, double lambda, double eta,
  ...
  if (rij > rcutij || rik > rcutik || rjk > rcutjk) {
    ...
    // dphi_dij
    dphi[0] =
        p2 * (dcosterm_dij * eterm * fcprod + costerm * determ_dij * fcprod +
              costerm * eterm * dfcprod_dij);
    // dphi_dik
    dphi[1] =
        p2 * (dcosterm_dik * eterm * fcprod + costerm * determ_dik * fcprod +
              costerm * eterm * dfcprod_dik);
    // dphi_djk
    dphi[2] =
        p2 * (dcosterm_djk * eterm * fcprod + costerm * determ_djk * fcprod +
              costerm * eterm * dfcprod_djk);
  }
}

void Descriptor::sym_g5(double zeta, double lambda, double eta, const double *r,
                        const double *rcut, double &phi) {
  double rij = r[0];
  double rik = r[1];
  double rjk = r[2];
...
void Descriptor::sym_d_g5(double zeta, double lambda, double eta,
  ...
  if (rij > rcutij || rik > rcutik) {
    ...
    double base = 1 + lambda * cos_ijk;
    if (base <=
        0) { // prevent numerical instability (when lambda=-1 and cos_ijk=1)
      costerm = 0.0;
      dcosterm_dcos = 0.0;
    } else {
      costerm = pow(base, zeta);
