...
public:
  ...
  int ComputeArgumentsCreate(KIM::ModelComputeArgumentsCreate
                                 *const modelComputeArgumentsCreate) const;
  int ComputeArgumentsDestroy(KIM::ModelComputeArgumentsDestroy
                                  *const modelComputeArgumentsDestroy) const;
...
private:
  ...
  int SetComputeMutableValues(
      ...
      bool &isComputeParticleVirial, int const *&particleSpeciesCodes,
      int const *&particleContributing, VectorOfSizeDIM const *&coordinates,
      double *&energy, VectorOfSizeDIM *&forces, double *&particleEnergy,
      VectorOfSizeSix *&virial, VectorOfSizeSix *&particleVirial);
  ...
  int Compute(KIM::ModelCompute const *const modelCompute,
              ...
              VectorOfSizeDIM *const forces, double *const particleEnergy,
              VectorOfSizeSix virial,
              VectorOfSizeSix *const particleVirial) const;

  // LJ functions
  void calc_phi(double const epsilon, double const sigma, double const cutoff,
                double const r, double *const phi) const;
...
int ANNImplementation::Compute(
    ...
    const VectorOfSizeDIM *const coordinates, double *const energy,
    VectorOfSizeDIM *const forces, double *const particleEnergy,
    VectorOfSizeSix virial, VectorOfSizeSix *const particleVirial) const {
  int ier = false;
  ...
  for (int i = 0; i < Nparticles; i++) {
    ...
    if (need_dE) {
      AllocateAndInitialize2DArray<double>(dGCdr, Ndescriptors,
                                           (numnei + 1) * DIM);
    }

    descriptor_->generate_one_atom(i, coordinates, particleSpeciesCodes, n1atom,
                                   numnei, GC, dGCdr[0], need_dE);
