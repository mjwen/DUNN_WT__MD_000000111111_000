...
Descriptor::Descriptor() { has_three_body = false; }
...
int Descriptor::read_parameter_file(FILE * const filePointer)
{
  ...
  for (int i = 0; i < numDescTypes; i++)
  {
    ...
    if (strcmp(name, "g1") == 0) { add_descriptor(name, NULL, 1, 0); }
    else
    {
  ...
  set_center_and_normalize(do_center_and_normalize, size, means, stds);
...
void Descriptor::set_cutoff(char * name, int ncutoff, double * value)
{
  ...
  for (int i = 0; i < ncutoff; i++) { cutoff.push_back(value[i]); }
}
